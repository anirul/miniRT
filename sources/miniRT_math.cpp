...
#ifdef _M_IX86
const _MM_ALIGN16 __int32 __MASKSIGNs_[4] = {0x80000000, 0x80000000, 0x80000000,
                                             0x80000000};
const _MM_ALIGN16 __int32 _Sign_PNPN[4] = {0x00000000, 0x80000000, 0x00000000,
                                           0x80000000};
const _MM_ALIGN16 __int32 _Sign_NPNP[4] = {0x80000000, 0x00000000, 0x80000000,
                                           0x00000000};
const _MM_ALIGN16 __int32 _Sign_PNNP[4] = {0x00000000, 0x80000000, 0x80000000,
                                           0x00000000};
const _MM_ALIGN16 __int32 __0FFF_[4] = {0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
                                        0x00000000};
...
#ifdef _M_IX86
    __asm {xorps xmm0, xmm0 mov eax, this;
fld float ptr rads movaps[eax + 0x10],
    xmm0                                   // clear line _L2
        fsincos fst float ptr[eax + 0x14]  // set element _22
    movaps[eax + 0x20],
    xmm0                            // clear line _L3
        fstp float ptr[eax + 0x28]  // set element _33
    fst float ptr[eax + 0x18]       // set element _23
    fchs movaps[eax + 0x00],
    xmm0                            // clear line _L1
        fstp float ptr[eax + 0x24]  // set element _32
    fld1 fst float ptr[eax + 0x00]  // set element _11
    movaps[eax + 0x30],
    xmm0                            // clear line _L4
...
#ifdef _M_IX86
    __asm {
        xorps xmm0, xmm0 mov eax, this fld float ptr rads movaps[eax + 0x00],
        xmm0                                   // clear line _L1
            fsincos fst float ptr[eax + 0x00]  // set element _11
        movaps[eax + 0x20],
        xmm0                            // clear line _L3
            fstp float ptr[eax + 0x28]  // set element _33
        fst float ptr[eax + 0x20]       // set element _31
        fchs movaps[eax + 0x10],
        xmm0                            // clear line _L2
            fstp float ptr[eax + 0x08]  // set element _13
        fld1 fst float ptr[eax + 0x14]  // set element _22
        movaps[eax + 0x30],
        xmm0                            // clear line _L4
            fstp float ptr[eax + 0x3C]  // set element _44
    }
...
void matrix::RotateZMatrix(const float rads){
    ...
    __asm {
        ...
        fchs movaps[eax + 0x20],
        xmm0                            // clear line _L3
            fstp float ptr[eax + 0x10]  // set element _21
        fld1 fst float ptr[eax + 0x28]  // set element _33
        movaps[eax + 0x30],
        xmm0                            // clear line _L4
            fstp float ptr[eax + 0x3C]  // set element _44
    }
...
//				translation.
// ----------------------------------------------------------
matrix TranslateMatrix(const float dx, const float dy, const float dz) {
  matrix ret;
  ret.IdentityMatrix();
  ret._41 = dx;
  ret._42 = dy;
...
void matrix::TranslateMatrix(const float dx, const float dy, const float dz) {
  IdentityMatrix();
  _41 = dx;
  _42 = dy;
  _43 = dz;
...
// ----------------------------------------------------------
matrix ScaleMatrix(const float a, const float b, const float c) {
  matrix ret;
  ret.IdentityMatrix();
  ret._11 = a;
  ret._22 = b;
...
void matrix::ScaleMatrix(const float a, const float b, const float c) {
  IdentityMatrix();
  _11 = a;
  _22 = b;
  _33 = c;
...
float matrix::MinValue() {
  ...
  for (int i = 0; i < 4; ++i)
    for (int j = 0; j < 4; ++j)
      if (this->operator()(i, j) < min) min = this->operator()(i, j);
  return min;
#endif  // _M_IX86
}
...
float matrix::MaxValue() {
  ...
  for (int i = 0; i < 4; ++i)
    for (int j = 0; j < 4; ++j)
      if (this->operator()(i, j) > max) max = this->operator()(i, j);
  return max;
#endif  // _M_IX86
}
...
float matrix::Inverse() {
  ...
  F32vec4 A = _mm_movelh_ps(_L1, _L2), B = _mm_movehl_ps(_L2, _L1),
          C = _mm_movelh_ps(_L3, _L4), D = _mm_movehl_ps(_L4, _L3);
  ...
  iB -= (F32vec4)_mm_mul_ps(_mm_shuffle_ps(D, D, 0xB1),
                            _mm_shuffle_ps(AB, AB, 0x66));
  //  iC = A * (D#C)# = A*C#*D
  iC = _mm_mul_ps(A, _mm_shuffle_ps(DC, DC, 0x33));
  iC -= (F32vec4)_mm_mul_ps(_mm_shuffle_ps(A, A, 0xB1),
