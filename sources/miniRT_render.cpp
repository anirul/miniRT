...
namespace miniRT {
	...
	unsigned int render::phong(
		const vector4& tuvi, const vector3& dir, int i) {
			...
			if (visible) {
				...
				if (last_occulted != i && last_occulted != -1) {
					...
					if (tri->shadow_hit(
						u0, u1, u2,
						hitpoint,
						incoming_light_normal,
						epsilon,
						sqrt((hitpoint - pl[j].position()) * 
						(hitpoint - pl[j].position())) - epsilon))
					{
						visible = false;
					}
				}
