...
// interface
struct IWin {
  virtual void key_down(int key, int x, int y, window* w) = 0;
  virtual void key_up(int key, int x, int y, window* w) = 0;
  virtual void mouse(int button, int x, int y, window* w) = 0;
  virtual void motion(int x, int y, window* w) = 0;
...
class window {
 ...
 public:
  ...
  bool create(unsigned int x, unsigned int y, unsigned int d = 32,
              bool f = false);
  void clear();
  bool run();
};
